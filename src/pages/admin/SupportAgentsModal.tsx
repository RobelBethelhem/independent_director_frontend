...
export function SupportAgentsModal({ onClose }: { onClose: () => void }) {
  ...
  async function load() {
    setRows(await adminApi.supportAgents());
  }
  useEffect(() => {
    void load();
  }, []);
  ...
  async function add() {
    ...
    setBusy(true);
    try {
      const res = await adminApi.createUser({ name, email, phone: phone || undefined, role: 'support' });
      ...
      setError(err instanceof HttpError ? err.messages.join(' · ') : 'Could not add support agent.');
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <Modal title="Support agents" width={640} onClose={onClose}>
      <p className="muted" style={{ marginTop: 0, marginBottom: 18, fontSize: 13 }}>
        Support agents staff the live help chat — they see applicants' and visitors' questions and reply with text, images,
        or voice notes from the Support console. Each agent gets their own sign-in and must set a new password on first login.
      </p>
      <div style={{ marginBottom: 22 }}>
        ...
        <div className="grid-2" style={{ marginBottom: 12 }}>
          <Field label="Full name" required>
            <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Hanna Girma" />
          </Field>
        ...
        <button className="btn btn-primary" disabled={busy} onClick={() => void add()}>
          <UserPlus size={16} /> {busy ? 'Creating…' : 'Create support agent & email credentials'}
        </button>
      </div>
      ...
      {!rows ? (
        ...
        <p className="muted">No support agents yet.</p>
      ) : (
